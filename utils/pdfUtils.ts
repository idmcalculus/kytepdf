...
export function parsePageRange(input: string, maxPages: number): number[] {
  ...
  for (const part of parts) {
    if (part.includes("-")) {
      ...
      if (!Number.isNaN(num) && num >= 1 && num <= maxPages) {
        pages.add(num);
      }
    }
  }
 ...
 */
export function filterValidFiles(files: FileList | File[], maxSizeMB: number = 100): File[] {
 ...
 */
export function buildJobCardHTML(
  job: JobData & { tool?: string; fileName?: string; fileSize?: number },
): string {
