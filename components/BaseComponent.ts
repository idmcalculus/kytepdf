...
export class BaseComponent extends HTMLElement {
  ...
  getDropZone(title = "your PDF", icon = "file-up", multiple = false) {
    ...
    return `
      ...
      <div id="storageWarning" class="warning hidden" style="margin-bottom: 1.5rem; border-color: var(--primary-glow); background: rgba(6, 182, 212, 0.05); color: var(--text-muted);">
        ...
        <span>Your session is automatically saved. To clear browser storage, <a href="#" id="clearStorageLink" style="color: var(--primary); text-decoration: underline;">click here</a>.</span>
      </div>
    `;
  }
      ...
      if (clearLink) {
        clearLink.onclick = async (e) => {
          ...
          const confirmed = await this.showConfirmDialog(
            "This will clear all saved progress and files from your browser. Are you sure?",
          );
          if (confirmed) {
            await this.clearFullStorage();
          }
        };
      }
  ...
  async clearFullStorage() {
    ...
    logger.info("Storage cleared and UI updated without reload");
  }

  async checkStorageUsage() {
    try {
      const { usage, quota } = await persistence.getStorageUsage();
      ...
      if (usage! > 0) {
        ...
        if (storageWarning) {
          ...
          if (span) {
            span.innerHTML = `Session auto-saved (${usageMB}MB used). <a href="#" id="clearStorageLink" style="color: var(--primary); text-decoration: underline;">Clear storage</a>`;
            ...
            if (clearLink) {
              clearLink.onclick = async (e) => {
                e.preventDefault();
                const confirmed = await this.showConfirmDialog(
                  "Clear all saved files from your browser?",
                );
                if (confirmed) {
                  await this.clearFullStorage();
                }
      ...
      if (usage! > quota! * 0.8) {
        logger.warn("Browser storage is almost full", { usage, quota });
