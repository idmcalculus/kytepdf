...
describe('Logger', () => {
	...
	it('should log error messages and call telemetry', () => {
		...
		expect(consoleSpy.error).toHaveBeenCalledWith('[KytePDF][ERROR]', 'test error', { detail: 'extra' });
		expect(telemetry.captureException).toHaveBeenCalledWith('test error', { details: { detail: 'extra' } });
	});
